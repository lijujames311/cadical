...
//
void Internal::remove_observed_var (Lit ilit) {
...
//
bool Internal::observed (Lit ilit) const {
...
#ifndef NDEBUG
  LOG ("(decision level: %" LEVEL ", trail size: %zd, notified %zd)", level,
       trail.size (), notified);
#endif
  renotify_full_trail ();
}
...
void Internal::renotify_full_trail_between_trail_pos (
...
#ifdef LOGGING
  LOG ("starting notification of level %" LEVEL " from trail %" LEVEL " .. %" LEVEL,
  ...
  for (; j < end_level; ++j) {
    ...
    LOG ("notifying elit %" LEVEL " @ %s aka %s", propagator_level, LOGLIT (elit),
         LOGLIT (ilit));
...
void Internal::renotify_full_trail () {
  ...
  for (int i = 2; i < c_size; ++i) {
    ...
    propagator_level++;
    LOG ("notification of %" LEVEL, propagator_level);
...
//
bool Internal::is_decision (Lit ilit) {
...
#ifndef NDEBUG
  LOG (v.reason,
       "is_decision: i%s (current level: %" LEVEL ", is_fixed: %d, v.level: %" LEVEL ", "
...
bool Internal::external_propagate () {
  ...
  while (cb_repropagate_needed && !conflict && external_prop &&
...
#ifndef NDEBUG
    LOG ("external propagation starts (decision level: %" LEVEL ", trail size: "
         "%zd, notified %zd)",
    ...
    int dimacs_elit = external->propagator->cb_propagate ();
    ELit elit;
    if (dimacs_elit)
    ...
    while (elit != INVALID_ELIT) {
      ...
      dimacs_elit = external->propagator->cb_propagate ();
      if (dimacs_elit)
        elit = ELit (dimacs_elit);
      else
        elit = INVALID_ELIT;
...
#ifndef NDEBUG
    LOG ("External propagation ends (decision level: %" LEVEL ", trail size: %zd, "
         "notified %zd)",
         level, trail.size (), notified);
#endif
    if (!unsat && !conflict) {
...
#ifndef NDEBUG
    LOG ("External clause addition ends on decision level %" LEVEL " at trail "
         "size "
         "%zd (notified %zd)",
         level, trail.size (), notified);
#endif
...
// newest_clause
//
void Internal::add_external_clause (ELit propagated_elit,
                                    bool no_backtrack) {
  assert (original.empty ());
  ELit elit = INVALID_ELIT;
  bool propagated_lit_found = false;

  if (propagated_elit != INVALID_ELIT) {
  ...
  while (elit != INVALID_ELIT) {
    ...
    if (propagated_elit != INVALID_ELIT) {
      elit =
          ELit (external->propagator->cb_add_reason_clause_lit (propagated_elit.signed_representation()));
      if (elit == propagated_elit)
        propagated_lit_found = true;
    } else
      elit = ELit (external->propagator->cb_add_external_clause_lit ());
...
//
void Internal::explain_reason (Lit ilit, Clause *reason, int &open) {
  ...
  for (const auto &other : *reason) {
    ...
    if (v.reason == external_reason) {
      v.reason = learn_external_reason_clause (-other, INVALID_ELIT, true);
    }
    if (v.level && v.reason) {
      f.seen = true;
      open++;
...
// argument.
//
Clause *Internal::learn_external_reason_clause (Lit ilit,
                                                ELit falsified_elit,
                                                bool no_backtrack) {
...
#ifndef NDEBUG
  if (falsified_elit == INVALID_ELIT && newest_clause) {
    ...
    // falsified clause.
    const Lit propagated_ilit = ilit;
...
//
Clause *Internal::wrapped_learn_external_reason_clause (Lit ilit) {
  Clause *res;
...
void Internal::handle_external_clause (Clause *res) {
  ...
  if (from_propagator)
    stats.ext_prop.elearned++;
  assert (res->size >= 2);
  const Lit pos0 = res->literals[0];
  const Lit pos1 = res->literals[1];
  ...
  const Var::Level l1 = var (pos1).level;
...
bool Internal::external_check_solution () {
  ...
  while (trail_changed || added_new_clauses) {
    ...
    // anymore.
    for (ELit::base_type idx = 1;
         idx <= std::min ((ELit::base_type) external->is_observed.size () - 1,
                          external->max_var);
         idx++) {
      if (!external->is_observed[idx])
        continue;
      const ELit lit = external->ival (ELit (idx));
      etrail.push_back (lit.signed_representation());
#ifndef NDEBUG
#ifdef LOGGING
  ...
  if (!unsat && conflict) {
    const Var::Level conflict_level = var (conflict->literals[0]).level;
...
Lit Internal::ask_decision () {
  ...
  Var::Level level_before = level;
  forced_backt_allowed = true;
  ELit elit = ELit (external->propagator->cb_decide ());
  forced_backt_allowed = false;
  stats.ext_prop.ext_cb++;
...
void Internal::mark_garbage_external_forgettable (int64_t id) {
  ...
  external->forgettable_original[id][0] = INVALID_ELIT;
}
...
bool Internal::get_merged_literals (std::vector<ELit::base_type> &eq_class) {
  ...
  if (!lit_level) {
    ...
    for (auto id : external->e2i) {
      ELit o_elit = id.first;
      Lit o_ilit = id.second;
      Lit other = o_ilit.labs ();
      if (other == ivar) {
        if (o_ilit == ilit)
          eq_class.push_back (o_elit.signed_representation());
        else
          eq_class.push_back (-o_elit.signed_representation());
      }
    }
