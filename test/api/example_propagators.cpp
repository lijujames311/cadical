...
//--------------------------------------------------------------------------//
// Some example implementations of the ExternalPropagator class are shown here. 
// In these examples we solve pigeonhole-type SAT problems by assigning exactly
// p_limit pigeons to n holes, such that each hole contains at most one pigeon
// and each pigeon belongs to exactly one hole. If p_limit > n, the problem is
// UNSAT; if p_limit <= n, it is SAT.
...
// Nevertheless, we use this problem here to keep the example relatively simple
// and avoid implementing complicated domain-specific propagators. 

// The main purpose of these examples is to demonstrate certain erroneous usage
// scenarios and highlight some common pitfalls (see 'Fatal error' comments).
// The implementation is simplified and provided as-is, without warranty or 
// guarantee to fit for any specific purpose beyond testing and demonstration.
...
static int p_id (int lit) {  
  return int((abs(lit) - 1) / n);
}

static int h_id (int lit) {  
  return ((abs(lit) - 1) % n);
}
...
public:
  ObserverPropagator (CaDiCaL::Solver *s) : solver(s) {
    ...
    //
    // The flag can be changed during solving, however switching it from true
    // to false will mean that the propagator does not know what is the actual
    // state of the trail in the solver (so it is not really recommended). 
    is_lazy = false;
    ...
    // See also the 'notify_fixed_assignment' function.
    solver->connect_fixed_listener (this);
    
    //Fatal error: can not connect more than one propagator
    //solver->connect_external_propagator (this);

    std::cout << "[obs-prop] ObserverPropagator is created and connected "\
    ...
    // thus it can trigger a fatal error if it is an already existing internal
    // variable. But in this example we have the simplified non-strict
    // interface.
    ...
    // When a unit clause is found by the SAT solver, notify_fixed_assignment
  ...
  ~ObserverPropagator () {
    ...
    std::cout << "[obs-prop] ObserverPropagator is disconnected." << std::endl;
  }
  ...
  // be notified that it has to be unassigned and then immediately reassigned 
  // upon every backtrack to a level higher than 0. Hence, following only the 
  // notify_assignment callbacks is correct, but requires more unnecessary work
  // on the propagator side. Using the FixedAssignmentListener notification
  // provides the necessary information to recognize and ignore these 
  // notifications in the use cases where it is relevant.
  void notify_fixed_assignment (int lit) {
    std::cout << "[fal][notify_fixed_assignment][" << lit << "]" << std::endl;
  }

  // Notification regarding the last batch of assignments made by the solver.
  // All of these assignments are happening in the current decision level and
  // in the order as they appear in lits. But, it is not guaranteed that the 
  // very first literal is a decision literal.
  // For example, if the decision literal is unobserved, no notification
  // regarding of its assignment is sent (only that a new level was started).
  // Also, there can happen multiple notifications on the same decision level
  // (consider for example cases where the interaction with the propagator
  // trigger further unit propagation steps in the SAT solver).
  ...
  void notify_new_decision_level () {
    ...
    // Fatal error: not allowed to force backtrack in that state of the solver
    // solver->force_backtrack(0);
  }

  void notify_backtrack (size_t new_level) {
    std::cout << "[obs-prop][notify_backtrack][ to level " << new_level << " ]" << std::endl;
  ...
  bool cb_check_found_model (const std::vector<int> &model) {
    ...
    // was forced, the return value of this function is ignored by the solver.
    if (first_model) {
      first_model = false;
      solver->force_backtrack(0);
      return true;
  ...
  int cb_decide () { 
    ...
    // force_backtrack is allowed to be called. However, the argument of this
    // forced backtrack must be a valid decision level. The following calls are
    // triggering fatal API errors:
    
    // Fatal error: the target level of a forced backtrack must be non-negative.
    // solver->force_backtrack(-6);
    
    // Fatal error: cadical: the target level of a forced backtrack must be smaller than the current decision level.
    ...
    // Fatal error: external decisions are only allowed over observed variables.
    // return non_observed_lit;

    // Fatal error: external decisions are only allowed over unassigned variables.
    // return fixed_lit;
  ...
  int cb_propagate () {
    ...
    // starts. If the new propagation agrees with an already existing internal
    // assignment, the external propagation step has no effect. If the
    // variable is not yet assigned, external propagation will enforce the
    // propagated assignment.

    // Fatal error: not allowed to force backtrack in that state of the solver
    // solver->force_backtrack(0);

    // Fatal error: external propagations are only allowed over observed variables.
    // return non_observed_lit;
    ...
    if (propagation_count == 6) {
      ...
      // 0 (because this variable was already assigned by the added unit clause
      // in the constructor) and triggers the corresponding 'notify_backtrack'
      // callback at the same time.
      solver->add_observed_var(non_observed_lit);

      // Thus, this external propagation step will take effect on the decision
      // level after backtracking (in this example level 0).
      // So if there are external propagation steps that require to on-the-fly 
      // observe already assigned variables, it is important to make sure that 
      // the propagation holds even after the SAT solver backtracked (the solver
      // can not check it since the reason of the propagation step is not given
      // here).
      // In general, the safest and easiest approach is to observe any
      // potentially relevant variables from the beginning on and add new
      // observed variables during solving only if they are fresh variables.
      return -non_observed_lit;
    }
  ...
  int cb_add_reason_clause_lit (int propagated_lit) {
    (void) propagated_lit;
    // In this example we have no used external propagations, so this function
...
public:
  ...
  // The actual assignment regarding each object. 1 indicates that the element
  // is selected, 0 indicates unknown (unassigned), and -1 indicates that it
  // is not selected under the current assignment.
  std::vector<int> amap;
  ...
  void assign(size_t pos, int val) {
    ...
    if (next_unassigned_pos == pos) {
      while (++next_unassigned_pos < amap.size() && amap[next_unassigned_pos]) {};
    }
  }

  void unassign(size_t pos) {
    assert (amap[pos]);
    if (amap[pos] > 0) p_count--;
    amap[pos] = 0;
    if (pos < next_unassigned_pos) next_unassigned_pos = pos;
  }
...
// based on the not pre-encoded part of the problem (the at most one pigeon per
// hole constraints).
//
// This example also shows a possible way to schedule the deletion of the not
// used reason clauses of external propagation steps (in case they are built
// upon propagation and not only when asked for them specificly).
...
private:
  ...
  // Used for storing the current external (reason) clauses during interactions
  // with the solver.
  std::vector<int> clause;
  size_t next_lit = 0;

  // In this example every external propagation is propagated from a binary
  // clause (one assigned pigeon forces each other pigeons to not get assigned
  // to the same hole).
  // We store explicitly the reason literals upon propagation, because 
  // recalculating on-the-fly the right reason would require us to track the
  // order of the assignments made.
...
public:
  ActivePropagator (CaDiCaL::Solver *s) : solver(s), hole_map(n, p_limit), pigeon_map(p_limit, n) {
    is_lazy = false;
    ...
    std::cout << "[act-prop] ActivePropagator is created and connected as " \
      "an ExternalPropagator" << std::endl;
    ...
    // Create the root-level of the trail. This level needs to be always there.
  ...
  ~ActivePropagator () {
    ...
    solver->disconnect_external_propagator(); 
    std::cout << "[act-prop] ActivePropagator is disconnected." << std::endl;
  }

  void notify_assignment (const std::vector<int> &lits) override {
    // We update the assignment states of the propagator, but in this example
    // we do not check eagerly if there is any possible conflict or propagation
    // (this will be checked only when it is relevant, ie., in 'cb_propagate' 
    // and 'cb_has_external_clause').
    ...
    for (const auto lit: lits) {
      ...
      // notification of assignment is actually about the reassignment of it,
      // hence we might need the reason clause of it later.
  ...
  void notify_backtrack (size_t new_level) override {
    std::cout << "[act-prop][notify_backtrack][ " << observed_trail.size() - 1 \
      << " -> " << new_level << " ]" << "[unassigned: ";
    ...
    while (observed_trail.size () > new_level + 1) {
      for (auto lit : observed_trail.top ()) {
        ...
        // In case it was a previously externally propagated literal, we mark
        // the reason clause of it for deletion, but do not delete it yet.
        // See 'notify_assignment' and 'cb_decide' for more details regarding
        // reason storing.
  ...
  int cb_add_reason_clause_lit (int propagated_lit) override {
    ...
    std::cout << "[act-prop][cb_add_reason_clause_lit][" \
      << propagated_lit << "][ ";
    
    // Fatal error: external reason clause must contain the propagated literal.
    // return 0;

    // Fatal error: external (reason) clause must contain only observed variables.
    // return non_observed_lit;

    // Fatal error: solver in invalid state
    // solver->disconnect_external_propagator ();
    
    if (clause.empty()) explain_propagation (propagated_lit);
    ...
    // Fatal error: can not unobserve assigned variable during conflict analysis
    // if (lit) {
